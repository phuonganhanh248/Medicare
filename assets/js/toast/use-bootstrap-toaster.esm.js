...
const toast = (input, BootstrapToast) => {
  ...
  if (Toaster === void 0) {
    ...
    const toast2 = Toaster.getOrCreateInstance(toastElement, {
      animation,
      autohide,
      delay
    });
...
toast.hide = (BootstrapToast) => {
  const Toaster = getToaster(BootstrapToast);
  if (Toaster === void 0) {
    throw new Error("Bootstrap Toast is not defined.");
  } else {
    document.body.querySelectorAll(":scope > .toast").forEach((el) => {
      const toast2 = Toaster.getOrCreateInstance(el);
      toast2.hide();
    });
  }
};
